...
def read_one_by_user_lname(lname):
    person = Person.query.filter(Person.lname == lname).one_or_none()
    if person is not None:
        notes = Note.query.filter(Note.person_id == person.id).all()
        if notes:
            return notes_schema.dump(notes)
        else:
            abort(404, f"Person with last name {person.lname} does not have any notes.")
    else:
